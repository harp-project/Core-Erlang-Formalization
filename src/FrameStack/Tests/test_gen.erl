...
generate_all() ->
    %% AST definitions
    FunGt0 = with_ast(fun(X) -> X > 0 end, ast_fun(1, ast_call("erlang", ">", [ast_var(0), ast_int(0)]))),
    FunGt2 = with_ast(fun(X) -> X > 2 end, ast_fun(1, ast_call("erlang", ">", [ast_var(0), ast_int(2)]))),
    FunEqB = with_ast(fun(X) -> X == b end, ast_fun(1, ast_call("erlang", "=:=", [ast_var(0), ast_atom(b)]))),
    FunEven = with_ast(fun(X) -> X rem 2 == 0 end, ast_fun(1, ast_call("erlang", "=:=", [ast_call("erlang", "rem", [ast_var(0), ast_int(2)]), ast_int(0)]))),
    FunWrapTrue = with_ast(fun(X) -> {true, X} end, ast_fun(1, ast_tuple([ast_atom(true), ast_var(0)]))),
    FunDupList = with_ast(fun(X) -> [X, X] end, ast_fun(1, ast_cons(ast_var(0), ast_cons(ast_var(0), ast_nil())))),
    FunPair = with_ast(fun(A, B) -> {A, B} end, ast_fun(2, ast_tuple([ast_var(0), ast_var(1)]))),
