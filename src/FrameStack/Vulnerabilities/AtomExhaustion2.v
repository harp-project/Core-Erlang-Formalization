...
Lemma mk_atom_set_union : forall (av: string) (l: SideEffectList),
  mk_atom_set ((AtomCreation, [VLit av]) :: l) = ({[av]} ∪ mk_atom_set l).
Proof.
...
Lemma mk_atom_set_unfit2 : forall (id: SideEffectId),
  ...
  mk_atom_set ((id, vl) :: l) = mk_atom_set l.
Proof.
  intros. destruct id.
  1-2: unfold mk_atom_set; reflexivity.
  contradiction.
...
Lemma galnua_multistep_summarise :
...
Proof.
  ...
  - destruct H2.
    ...
    + apply (step_determenism H) in H2. destruct H2, H5.
      ...
      rewrite <- H2 in IHstep_rt.
      assumption.
Admitted.

Lemma galnua_multistep_summarise_rev :
  forall fs r fs' r' l, ⟨ fs , r ⟩ -[ l ]->* ⟨ fs' , r' ⟩ ->
