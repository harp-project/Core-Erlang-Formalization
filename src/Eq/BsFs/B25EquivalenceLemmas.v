From CoreErlang.Eq.BsFs Require Export B24ValueReductionLemmas.
...
Section Atoms1.
  ...
  Theorem eq_bs_to_fs_suc_nil :
    forall fns r env,
        bresult_to_fresult fns (inl [VNil]) = r
  ...
  Theorem eq_bs_to_fs_suc_lit :
    forall fns r env lit,
        bresult_to_fresult fns (inl [VLit lit]) = r
...
Section Atoms2.
  ...
  Theorem eq_bs_to_fs_suc_var :
    ...
    ->  fs_wfm_result r
    ->  bresult_to_fresult fns (inl vs) = r
  ...
  Proof.
    ...
    bse - value_reduction: v fns v' Hwfm_v Heq_v.
  Qed.
  ...
  Theorem eq_bs_to_fs_su2_funid :
    forall fns r env fid func own_module modules,
        get_own_modfunc own_module fid.1 fid.2 (modules ++ stdlib) = Some func
    ->  ⟨ [], bexp_to_fexp_subst fns env (EFunId fid) ⟩ -->* r.
  Proof.
    (* #1 Congruence: intro/specialize/congruence *)
    itr - fns r env fid func own_module modules Hmod.
    ass - as Hno_modfunc: adm > 
      ((forall fid,
...
Section Closures.
  ...
  Theorem eq_bs_to_fs_suc_fun :
    forall fns r env e vars id,
        bresult_to_fresult fns (inl [VClos env [] id vars e None]) = r
    ->  ⟨ [], bexp_to_fexp_subst fns env (EFun vars e) ⟩ -->* r.
  Proof.
    (* #0 Assert: intro/assert/admit *)
    itr - fns r env e vars id Hresult.
    ass - as Hscope: adm > (is_result r).
    (* #1 Inversion Result: inversion + subst/clear *)
    ivc - Hresult.
  ...
  Theorem eq_bs_to_fs_suc_letrec :
    forall fns r env e ext result id,
        (forall fns r,
            fs_wfm_result r
        ->  bresult_to_fresult fns result = r
        ->  ⟨ [], bexp_to_fexp_subst fns (append_funs_to_env ext env id) e ⟩
              -->* r)
    ->  fs_wfm_result r
    ->  bresult_to_fresult fns result = r
    ->  ⟨ [], bexp_to_fexp_subst fns env (ELetRec ext e) ⟩ -->* r.
  Proof.
    (* #1 Inversion Result: intro/inversion + subst/clear *)
    itr - fns r env e ext result id Hfs Hwfm Hresult.
    ivc - Hresult.
    (* #2 Simplify: cbn/unfold *)
    rfl - bexp_to_fexp_subst measure_env_exp measure_exp.
    smp.
    rfl - add_exp_ext_both.
    rwr - add_exp_ext_fids_smp.
    (* #3 Measure Reduction *)
    rwr - mred_eext_ext_rem_both
          mred_eext_e_rem_exp_ext_fids.
    (* #3 Specalize *)
    spc - Hfs: fns (bresult_to_fresult fns result) Hwfm.
    spe_rfl - Hfs.
    (* #4 Destruct *)
    des - Hfs as [k [Hscope Hstep]].
    (* #5 Rewrite Add Fids: rewrite *)
...
Section Doubles1.
  ...
  Theorem eq_bs_to_fs_suc_cons :
    forall fns r env e1 e2 v2 v1,
        (forall fns r,
            fs_wfm_result r
        ->  bresult_to_fresult fns (inl [v2]) = r
        ->  ⟨ [], bexp_to_fexp_subst fns env e2 ⟩ -->* r)
    ->  (forall fns r,
            fs_wfm_result r
        ->  bresult_to_fresult fns (inl [v1]) = r
        ->  ⟨ [], bexp_to_fexp_subst fns env e1 ⟩ -->* r)
    ->  fs_wfm_result r
    ->  bresult_to_fresult fns (inl [VCons v1 v2]) = r
  ...
  Proof.
    ...
    step - Hstep_v2 / kv2 e2.
    step - Hstep_v1 / kv1 e1 env fns.
    step.
  Qed.
  ...
  Theorem eq_bs_to_fs_ex1_cons :
    ...
    ->  (forall fns r,
            fs_wfm_result r
        ->  bresult_to_fresult fns (inr exc) = r
        ->  ⟨ [], bexp_to_fexp_subst fns env e1 ⟩ -->* r)
    ->  fs_wfm_result r
    ->  bresult_to_fresult fns (inr exc) = r
    ->  ⟨ [], bexp_to_fexp_subst fns env (ECons e1 e2) ⟩ -->* r.
  Proof.
    (* #0 Assert: intro/assert *)
    ...
    app - fs_wfm_val_to_result in Hwfm_v2.
    ufl - bresult_to_fresult in Hwfm_exc.
    (* #4 Remember Values: Remember Values *)
    ...
    scope - Hscope_exc.
    step - Hstep_v2 / kv2 e2.
    step - Hstep_exc / kexc e1 env fns.
    step.
  Admitted.
  ...
  Proof.
    ...
    (* #7 FrameStack Proof: scope/step + clear *)
    scope - Hscope_v2.
    step - Hstep_v1 / kv1 e1.
    step - Hstep_v2 / kv2 e2 env fns v1'.
    step.
  Admitted.
...
Section Doubles2.
  ...
  Proof.
    ...
    scope - Hscope_v2.
    step - Hstep_v1 / kv1 e1.
    step: rwr - Heq_v1 Hlength; bse - bvs_to_fvs_length
      / Hlength Heq_v1 v1.
    step - Hstep_v2.
  Admitted.
  ...
  Proof.
    ...
    scope - Hscope_v2.
    step - Hstep_v1 / kv1 e1.
    step: rwr - Heq_v1 Hlength; ase - bvs_to_fvs_length
...
Section Lists1.
  ...
  Theorem eq_bs_to_fs_nil_tuple :
    ...
    ->  ⟨ [], bexp_to_fexp_subst fns env (ETuple []) ⟩
          -->* bresult_to_fresult fns (inl [VTuple vl]).
  Proof.
  ...
  Theorem eq_bs_to_fs_suc_tuple :
    ...
    ->  (forall i,
        ...
        ->  (forall fns r,
                fs_wfm_result r
            ->  bresult_to_fresult fns (inl [nth i vl ErrorValue]) = r
            ->  ⟨ [], bexp_to_fexp_subst fns env (nth i el ErrorExp) ⟩ -->* r))
    ->  fs_wfm_result r
    ->  bresult_to_fresult fns (inl [VTuple vl]) = r
  ...
  Proof.
    ...
    pse - biforall_vtuple_nth_eq as Hbiforall:
    ...
    ass - as Heq_nth: smp; rwr - Heq_v >
      (bresult_to_fresult fns (inl [nth 0 (v :: vl) ErrorValue]) = RValSeq [v']);
      clr - Heq_v.
  ...
  Proof.
    ...
    itr - fns r env el vl exc j Hl_el Hlength_vl Hfs_nth Hfs_exc Hwfm_exc
          Hresult.
    ass - as Hwfm_vvl : adm >
      (fs_wfm_result (bresult_to_fresult fns (inl [VTuple vl]))).
    (* #1 Inversion Result: inversion + subst/clear *)
    ivc - Hresult.
    (* #2 Split List: pose/destruct/rewrite + clear *)
    pse - length_lt_split as Hsplit_el: Value Expression vl el Hl_el.
    ...
    * (* #4.1 Formalize List: clear/apply/inversion/rewrite + subst/clear *)
      ...
      spc - Hfs_exc: fns (bresult_to_fresult fns (inr exc)) Hwfm_exc.
      spe_rfl - Hfs_exc.
      smp - Hfs_exc.
    ...
    * (* #4.2 Formalize List: destruct/rewrite + inversion/subst *)
      ...
      rwr - mred_eel_e
            mred_eel_el.
      simpl bresult_to_fresult in Hfs_exc, Hwfm_exc.
  ...
  Theorem eq_bs_to_fs_suc_values :
    ...
    ->  (forall i,
        ...
        ->  (forall fns r,
            ...
            ->  ⟨ [], bexp_to_fexp_subst fns env (nth i el ErrorExp) ⟩ -->* r))
    ->  fs_wfm_result r
    ->  bresult_to_fresult fns (inl vl) = r
  ...
  Proof.
    ...
    pse - biforall_values_nth_eq as Hbiforall:
      fns env e el v vl v' vl'
      Heq_v Heq_vl Hlength Hwfm Hfs_nth;
      clr - Hlength Hwfm Heq_vl.
    pose proof ident_result
  ...
  Proof.
    ...
    itr - fns r env el vl exc j Hl_el Hlength_vl Hfs_nth Hfs_exc Hwfm_exc
          Hresult.
    ass - as Hwfm_vvl : adm >
      (fs_wfm_result (bresult_to_fresult fns (inl vl))).
    (* #1 Inversion Result: inversion + subst/clear *)
    ivc - Hresult.
    (* #2 Split List: pose/destruct/rewrite + clear *)
    pse - length_lt_split as Hsplit_el: Value Expression vl el Hl_el.
    ...
    * (* #4.2 Formalize List: destruct/rewrite + inversion/subst *)
      ...
      rem - v' vl' exc' as Heq_v Heq_vl Heq_exc:
        ...
        (bexc_to_fexc fns exc).
      (* #8.2 Pose Ident Theorem: pose + clear *)
      pse - biforall_values_nth_le as Hbiforall:
        fns env e1 el1 (e2 :: el2) v vl v' vl'
        Heq_v Heq_vl Hlength_vl Hwfm_vvl Hfs_nth;
        clr - Heq_vl Hlength_vl Hwfm_vvl.
      pose proof ident_partial
...
Section Main.
  ...
  Proof.
    ...
    3:  bse - eq_bs_to_fs_su1_funid:  fns r env fid res
                                      H Hwfm Hresult.
    3:  bse - eq_bs_to_fs_su2_funid:  fns r env fid func own_module modules
                                      H0.
    ...
    1: bse - eq_bs_to_fs_exc_values:   fns r env exps vals ex i
                                       H H0 H4 IHbs Hwfm Hresult.
    15: bse - eq_bs_to_fs_ex1_app: fns r env exp params ex IHbs Hwfm Hresult.
    (* #9 Applications: [e;el] (App) *)
    all: admit.
  Admitted.
