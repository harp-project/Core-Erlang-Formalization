...
Section Remove_Environment_Definitions.
  ...
  Definition rem_expext_vars
    (ext : list (FunctionIdentifier * FunctionExpression))
    (vars : list Var)
    (env : Environment)
    : Environment
    :=
  rem_expext ext (rem_vars vars env).
  ...
  Definition rem_pats
    ...
    : Environment
    :=
  rem_vars (vars_of_pattern_list pats) env.
...
Section Erase_Names.
  ...
  | ECons e1 e2 =>
    Syntax.ECons
      (erase_exp fns env e1)
      (erase_exp fns env e2)
  ...
  | EFun vars e =>
    Syntax.EFun
      (length vars)
      (erase_exp (add_vars vars fns) (rem_vars vars env) e)

  | ELet vars e1 e2 =>
    Syntax.ELet
      (length vars)
      (erase_exp fns e1)
      (erase_exp (add_vars vars fns) (rem_vars vars env) e2)

  | ETry e1 vars1 e2 vars2 e3 =>
    Syntax.ETry
      (erase_exp fns e1)
      (length vars1)
      (erase_exp (add_vars vars1 fns) (rem_vars vars1 env) e2)
      (length vars2)
      (erase_exp (add_vars vars2 fns) (rem_vars vars2 env) e3)
    ...
    Syntax.ELetRec
      (map
        (fun '(_, (vars, body)) =>
          (length vars,
          erase_exp
            (add_expext_vars ext vars fns)
            (rem_expext_vars ext vars env)
            body))
        ext)
      (erase_exp (add_expext ext fns) (rem_ext ext env) e)
  ...
  | ECase e case =>
    Syntax.ECase
      ...
      (map 
        (fun '(pats, e1, e2) =>
          ((map erase_pat pats),
          erase_exp (add_pats pats fns) (rem_pats pats env) e1,
          erase_exp (add_pats pats fns) (rem_pats pats env) e2))
        case)
