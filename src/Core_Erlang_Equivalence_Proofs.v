...
Example call_comm : forall (e e' : Expression) (x1 x2 t : Value) 
  ...
  |env, id, ECall (ELit (Atom "erlang" )) (ELit (Atom "+" )) [e ; e'], []| -e> | id, inl [t], []| ->
  |env, id, ECall (ELit (Atom "erlang" )) (ELit (Atom "+" )) [e' ; e], []| -e> | id, inl [t], []|.
Proof.
  intros. 
  (* List elements *)
  ...
  destruct H11. subst.
  simpl in P2 , H11, H13.
  rewrite <- H11, <- H13 in P2. subst.
  apply WD2 in P2. inversion P2. destruct H15.
  inversion H10. inversion H14. rewrite <- H15, <- H19, H16 in *. subst. 
  eapply eval_call with (vals := [x3; x]) (eff := [[];[]]) (ids := [nth 1 ids 0; nth 1 ids 0]); eauto.
  ...
  * rewrite (@plus_comm_basic x x3 t). 
      ...
      - simpl last.
        ...
        subst. inversion H6.
        pose (EE4 := element_exist _ _ H20).
        inversion EE4. inversion H19.
        subst. inversion H6. apply eq_sym, length_zero_iff_nil in H22. subst.
        simpl in H15, H9, H14. subst.
        exact H17.
Qed.
...
Example let_1_comm (e1 e2 : Expression) (t x1 x2 : Value) (id : nat) :
  ...
  |[], id, ELet ["X"%string] e1 (ECall (ELit (Atom "erlang" )) (ELit (Atom "+" )) [EVar "X"%string ; e2]), []| 
  -e> | id, inl [t], []| ->
  |[], id, ELet ["X"%string] e1 (ECall (ELit (Atom "erlang" )) (ELit (Atom "+" )) [e2 ; EVar "X"%string]), []| 
  -e> |id, inl [t], []|.
Proof.
...
Proof.
 ...
 (* FROM CALL HYPO1 *)
  ...
  pose (EC3 := element_exist _ _ H41). inversion EC3 as [eff10]. inversion H45. subst.
  inversion H41.
  pose (EC4 := element_exist _ _ H48). inversion EC4 as [eff20]. inversion H46. subst.
  inversion H41.
  apply eq_sym, length_zero_iff_nil in H50. subst.
  pose (EC5 := element_exist _ _ H42). inversion EC5 as [id10]. inversion H49. subst.
  inversion H42.
  pose (EC6 := element_exist _ _ H51). inversion EC6 as [id20]. inversion H50. subst.
  inversion H42.
  apply eq_sym, length_zero_iff_nil in H55. subst.
  (* FROM CALL HYPO2 *)
  inversion H28. subst.
  pose (EC1' := element_exist _ _ H57). inversion EC1' as [v20']. inversion H54. subst.
  inversion H57.
  pose (EC2' := element_exist _ _ H56). inversion EC2' as [v10']. inversion H55. subst.
  inversion H57.
  apply eq_sym, length_zero_iff_nil in H63. subst.
  pose (EC3' := element_exist _ _ H58). inversion EC3' as [eff20']. inversion H62. subst.
  inversion H58.
  pose (EC4' := element_exist _ _ H65). inversion EC4' as [eff10']. inversion H63. subst.
  inversion H58.
  apply eq_sym, length_zero_iff_nil in H67. subst.
  pose (EC5' := element_exist _ _ H59). inversion EC5' as [id20']. inversion H66. subst.
  inversion H59.
  pose (EC6' := element_exist _ _ H68). inversion EC6' as [id10']. inversion H67. subst.
  inversion H59.
  apply eq_sym, length_zero_iff_nil in H72. subst.
  
  Check plus_effect_unmodified. 
  pose (PUM1 := plus_effect_unmodified _ _ _ H72).        
  pose (PUM2 := plus_effect_unmodified _ _ _ H65).              
  inversion PUM1. inversion PUM2. simpl in H67, H68. subst.   
  
  (* EVERYTHING IS EQUAL *)
  (* assert (v1' = v1 /\ v1'' = v1 /\ v2' = v2 /\ v2'' = v2).
  { *)
    pose (P1 := H47 0 Nat.lt_0_2).
    pose (P2 := H47 1 Nat.lt_1_2).
    pose (P1' := H64 1 Nat.lt_1_2).
    pose (P2' := H64 0 Nat.lt_0_2).
    ...
    rewrite get_value_there in H77, H101.
     2-3 : congruence. 
    rewrite get_value_here in H77, H85 , H93, H101.
    inversion H77. inversion H85. inversion H101. inversion H93. subst. admit.
(* } *)
  (* clear PUM1. clear PUM2. ---GYURI---*) 
  (* apply (plus_comm_basic_value _ (eff ++ eff2 ++ eff1)) in H52. ---GYURI---*)
(* (  simpl last in H65.
  (* TODO: question!!!*)
  rewrite H50 in H65. inversion H65.
  reflexivity.) *)
...
(*     } *)
   ...
   (* call information *)
     + inversion H20. subst.
       pose (EC1 := element_exist 1 _ H13).
       pose (EC2 := element_exist 1 _ H16).
       pose (EC3 := element_exist 1 _ H18).
       inversion EC1 as [x'']. inversion EC2 as [eff1'']. inversion EC3 as [id1''].
       inversion H8. inversion H10. inversion H11. subst.
       inversion H13. inversion H16. inversion H18.
       ...
       inversion H24. inversion H26. inversion H27.
       apply eq_sym, length_zero_iff_nil in H33.
       ...
       apply eq_sym, length_zero_iff_nil in H35. subst.
         ...
         inversion P1'. inversion P2'. simpl in H44, H45, H43, H47. subst.
       ...
       **  admit.
       ** admit.
       ** intros. inversion H32. 2: inversion H34.
       ...
       ** apply plus_comm_basic_value with (eff0 := eff ++ eff1 ++ eff2).
          simpl in H37, H39. subst. admit. (* assumption.*)
Admitted. (* Qed. *)
...
(*     } *)
   ...
   (* call information *)
     + inversion H20. subst.
       pose (EC1 := element_exist 1 _ H13).
       pose (EC2 := element_exist 1 _ H16).
       pose (EC3 := element_exist 1 _ H17).
       inversion EC1 as [x'']. inversion EC2 as [eff1'']. inversion EC3 as [id1''].
       inversion H8. inversion H10. inversion H11. subst.
       inversion H13. inversion H16. inversion H17.
       pose (EC1' := element_exist 0 _ H23).
       pose (EC2' := element_exist 0 _ H25).
       pose (EC3' := element_exist 0 _ H26).
       inversion EC1' as [x0'']. inversion EC2' as [eff2'']. inversion EC3' as [id2''].
       inversion H22. inversion H27. inversion H28. subst.
       inversion H13. inversion H16. inversion H17.
       apply eq_sym, length_zero_iff_nil in H32.
       apply eq_sym, length_zero_iff_nil in H33.
       apply eq_sym, length_zero_iff_nil in H34. subst.

         pose (P1' := H24 0 Nat.lt_0_2).
         pose (P2' := H24 1 Nat.lt_1_2).
         inversion P1'. inversion P2'. simpl in H36, H38, H45, H40, H43, H44, H46, H37, H45. subst.

         rewrite get_value_there in H37. 2: congruence.
         rewrite get_value_here in H37. inversion H37.
         rewrite get_value_here in H45. inversion H45. subst.
       ...
       ** intros. inversion H31. 2: inversion H33.
         ...
         -- simpl. eapply eval_var. rewrite get_value_here. auto.
         -- inversion H35.
       ** apply plus_effect_changeable with (eff0 := eff ++ eff1 ++ eff2). admit. (*assumption.*)
  * intros. inversion H3. subst.
    pose (EE1 := element_exist 0 vals H14).
    inversion EE1 as [x'].
    inversion H4. subst.
   ...
   (* call information *)
     + inversion H20. subst.
       ...
       apply eq_sym, length_zero_iff_nil in H34. subst.
         ...
         rewrite get_value_there in H45. 2: congruence.
         rewrite get_value_here in H45. inversion H45.
       ...
       ** intros. inversion H31. 2: inversion H33.
         ...
         -- simpl. eapply eval_var. rewrite get_value_there, get_value_here. auto. congruence.
         -- inversion H35.
       ** apply plus_effect_changeable with (eff0 := eff ++ eff2 ++ eff1). admit. (*assumption.*)
Admitted. (*Qed.*)
