...
(*   symClos (preCompatibleNodes O) A B ->
  ...
  (forall A' a ι,
      A -[a | ι]ₙ-> A' with O ->
        exists B' l,
          B -[l]ₙ->* B' with O /\ barbedSim O (* n *) A' B') ->
  (forall source dest,
      dest ∈ O ->
      exists source' l B',
      B -[l]ₙ->* B' with O /\
...
Notation "A ~≤~ B 'observing' O" := (barbedSim O (* n *) A B) (at level 70).

Theorem barbedBisim_barbedSim :
  forall O A B,
    A ~ B observing O ->
    barbedSim O A B.
Proof.
...
Theorem barbedBisim_barbedSim_sym :
  forall O A B,
    A ~ B observing O ->
    barbedSim O B A.
Proof.
  intros. apply barbedBisim_sym in H.
