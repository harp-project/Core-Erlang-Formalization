...
Section EraseSubstRemove_Lemmas.
...
(*
˝ match shift sbt1 n1 with
  | inl exp => exp
  | inr num => VVar num
  end = ˝ match shift sbt2 n2 with
          | inl exp => exp
          | inr num => VVar num
          end
*)
  ...
  Proof.
    intros e1 e2 HR.
    gen - e2.
    induction e1 using Exp_ind with
      (PV := Forall P).
    ind ~ Exp_ind - e1.
  ...
  induction v1 using Val_ind with
    (P := fun v => forall v2, renameVal S v = renameVal S v2 -> v = v2);
    intros v2 H; destruct v2; try (simpl in H; inversion H; subst; reflexivity).
  - des - n; ivs - H.
  - des - n; ivs - H.
  - des - n; ivs - H.
  - ivs - H.
  ...
  - (* Case VMap *)
    ...
    f_equal. apply map_ext_in. intros [x1 y1] H_in.
    apply IHv1_1. simpl. reflexivity.
  - (* Case VVar *)
  ...
  - (* Case VClos *)
    simpl in H. inversion H. subst.
    f_equal.
    + apply map_ext_in. intros [i1 ls1 x1] H_in.
      apply IHv1_1. simpl. reflexivity.
    + reflexivity.
    + reflexivity.
    + apply IHv1_2. simpl. reflexivity.
Qed.
    itr.
    ind - v1.
    * des - v2 :- ivs - H.
  ...
  Proof.
    ...
    * rwr - cons_app.
      ...
      - des > ((x =? x')%string)
        ...
        smp + IHks.
        rwr - app_nil_r in *.
        rem - sbt1 sbt2 as Hsbt1 Hsbt2:
          (upn (base.length ks' + 1)
               (list_subst (map erase_val ((k₁, v₁) -ᵏ k).vals) ξ))
          (upn (base.length ks' + 1) (erase_val v₁ .: ξ)).
        rem - n1 n2 as Hn1 Hn2:
          (apply_eraser (ks' ᵏ++ (k :: ((k₁, v₁) -ᵏ k).keys ᵏ++ σ)) (inl x))
          (apply_eraser (ks' ᵏ++ (k :: k₁ :: σ)) (inl x)).
        Search "rewrite".
        setoid_rewrite <- Hn2.
        setoid_rewrite <- Hn2 in IHks.
        Print shift.
    (*     exa - IHks.
    des > (k =ᵏ k₁) as Heq_k.
    * rwr - var_funid_eqb_eq in Heq_k.
