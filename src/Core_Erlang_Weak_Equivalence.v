...
Proof.
  ...
  * destruct (fbs_expr clock env id e2 eff) eqn:D1.
    destruct res0. destruct v. congruence. destruct v0. 2, 4-5: congruence.
    - apply IHclock in D1. destruct D1.
      destruct (fbs_expr clock env id0 e1 eff0) eqn:D2.
  ...
  * destruct e; subst.
    ...
    - simpl in H. simpl.
      ...
      + remember D1 as D1'. clear HeqD1'.
        ...
        destruct (fbs_expr clock env id0 e1 (eff ++ x)) eqn:D2.
        destruct res0. destruct v0. congruence. destruct v1. 2: congruence.
        3-4: congruence.
    ...
    - simpl. simpl in H. destruct (fbs_values (fbs_expr clock) env id l eff) eqn:D1.
      ...
      + inversion H. subst.
        eapply effect_irrelevant_exprlist_helper in D1. rewrite D1. reflexivity.
        intros. eapply IHclock. exact H0.
    - simpl. simpl in H. destruct (fbs_expr clock env id e eff) eqn:D1.
...
->
  weakly_equivalent_expr (ECons hd tl) (ECons hd' tl').
Proof.
  assert (A : forall hd hd' tl tl' env id eff id' res eff',
    weakly_equivalent_expr hd hd' -> weakly_equivalent_expr tl tl' ->
    (exists clock, fbs_expr clock env id (ECons hd tl) eff = Result id' res eff')
  ->
    (exists eff'', (exists clock, fbs_expr clock env id (ECons hd' tl') eff = Result id' res eff'') /\ Permutation eff' eff'')
  ). {
...
Theorem ETuple_weak_congr (l : list Expression) : forall (l' : list Expression),
  weakly_equivalent_exprlist l l'
->
  weakly_equivalent_expr (ETuple l) (ETuple l').
Proof.
  assert (A : forall (l l' : list Expression) env id eff id' res eff',
    ...
    ->
      exists eff'', (exists clock, fbs_expr clock env id (ETuple l') eff = Result id' res eff'') /\ Permutation eff' eff''). 
  {
...
Theorem EApp_weak_congr (exp : Expression) (l : list Expression) : forall (exp' : Expression) (l' : list Expression),
  ...
  weakly_equivalent_exprlist l l'
->
  weakly_equivalent_expr (EApp exp l) (EApp exp' l').
Proof.
  assert (A : forall exp exp' (l l' : list Expression) env id eff id' res eff',
    ...
    ->
    exists eff'', (exists clock, fbs_expr clock env id (EApp exp' l') eff = Result id' res eff'') /\ Permutation eff' eff'').
  {
...
Theorem ECase_weak_congr (exp : Expression) (l : list (list Pattern * Expression * Expression)) : forall (exp' : Expression) (l' : list (list Pattern * Expression * Expression)),
  ...
  weakly_equivalent_case l l'
->
  weakly_equivalent_expr (ECase exp l) (ECase exp' l').
Proof.
  ...
  ->
    (exists eff'', (exists clock,
                   fbs_expr clock env id (ECase exp' l') eff = Result id' res eff'') /\
                   Permutation eff' eff'') 
  ).
  {
    intros. destruct H1. destruct x. inversion H1. simpl in H1.
...
->
  weakly_equivalent_expr (ELet vl e1 e2) (ELet vl e1' e2').
Proof.
  ...
  ->
    (exists eff'', (exists clock, fbs_expr clock env id (ELet vl e1' e2') eff = Result id' res eff'') /\ Permutation eff' eff'')
  ).
  {
...
Theorem ESeq_weak_congr e1 e2 e1' e2' :
  weakly_equivalent_expr e1 e1' -> weakly_equivalent_expr e2 e2'
->
  weakly_equivalent_expr (ESeq e1 e2) (ESeq e1' e2').
Proof.
  assert (A : forall env id e1 e1' e2 e2' eff id' res eff',
    weakly_equivalent_expr e1 e1' -> weakly_equivalent_expr e2 e2' ->
    (exists clock, fbs_expr clock env id (ESeq e1 e2) eff = Result id' res eff')
  ->
    (exists eff'', (exists clock, fbs_expr clock env id (ESeq e1' e2') eff = Result id' res eff'') /\ Permutation eff' eff'')
  ).
  {
    intros. destruct H1, x. inversion H1. simpl in H1.
    destruct (fbs_expr x env id e0 eff) eqn:D1.
...
Theorem ELetRec_weak_congr (exp : Expression) l : forall (exp' : Expression),
  weakly_equivalent_expr exp exp'
->
  weakly_equivalent_expr (ELetRec l exp) (ELetRec l exp').
Proof.
  intros.
  split; intros.
...
->
  weakly_equivalent_expr (EMap l) (EMap l').
Proof.
  assert (A :
    ...
    ->
    (exists eff'', (exists clock, fbs_expr clock env id (EMap l') eff = Result id' res eff'') /\ Permutation eff' eff'')
  ). {
...
->
  weakly_equivalent_expr (ETry e1 vl1 e2 vl2 e3) (ETry e1' vl1 e2' vl2 e3').
Proof.
  ...
  ->
    (exists eff'', (exists clock, fbs_expr clock env id (ETry e1' vl1 e2' vl2 e3') eff = Result id' res eff'') /\ Permutation eff' eff'')
  ).
  {
...
Theorem EValues_weak_congr (l : list Expression) : forall (l' : list Expression),
  weakly_equivalent_exprlist l l'
->
