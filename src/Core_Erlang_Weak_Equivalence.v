...
->
  (exists eff'', (exists clock, fbs_expr clock env modules id1 e2 eff = Result id2 res eff'') /\ Permutation eff' eff'')
)
/\
(forall env modules eff res eff' id1 id2, P modules -> 
  (exists clock, fbs_expr clock env modules id1 e2 eff = Result id2 res eff')
->
  (exists eff'', (exists clock, fbs_expr clock env modules id1 e1 eff = Result id2 res eff'') /\ Permutation eff' eff'')).
...
->
  (exists eff'', (exists clock, fbs_values (fbs_expr clock) env modules id1 l2 eff = Result id2 res eff'') /\ Permutation eff' eff'')
)
/\
(forall env modules eff res eff' id1 id2, P modules ->
  (exists clock, fbs_values (fbs_expr clock) env modules id1 l2 eff = Result id2 res eff')
->
  (exists eff'', (exists clock, fbs_values (fbs_expr clock) env modules id1 l1 eff = Result id2 res eff'') /\ Permutation eff' eff'')).
...
->
  (exists eff'', (exists clock, fbs_case l2 env modules id1 eff vals (fbs_expr clock)= Result id2 res eff'') /\ Permutation eff' eff'')
)
/\
(forall env modules eff res eff' id1 id2 vals, P modules ->
  (exists clock, fbs_case l2 env modules id1 eff vals (fbs_expr clock) = Result id2 res eff')
->
  (exists eff'', (exists clock, fbs_case l1 env modules id1 eff vals (fbs_expr clock) = Result id2 res eff'') /\ Permutation eff' eff'')).
...
forall {env modules id0 eff id' res eff' clock},
(forall (env : Environment) (id : nat) (e : Expression)
                          (eff : SideEffectList) (id' : nat) (res : ValueSequence + Exception)
                          (eff' : SideEffectList),
                        fbs_expr clock env modules id e eff = Result id' res eff' ->
                        exists l : list (SideEffectId * list Value), eff' = eff ++ l)
  ->
fbs_values (fbs_expr clock) env modules id0 el eff = Result id' res eff'
->
exists l, eff' = eff ++ l.
Proof.
  induction el; intros.
  ...
  * simpl in H0.
    destruct (fbs_expr clock env modules id0 a eff) eqn:D.
    destruct res0. destruct v. congruence. destruct v0. 2: congruence.
    3-4: congruence.
    - apply H in D.
      destruct D. subst.
      destruct (fbs_values (fbs_expr clock) env modules id el (eff ++ x)) eqn:D2.
...
forall {env modules id0 eff id' res eff' vals clock},
(forall (env : Environment) (id : nat) (e : Expression)
                          (eff : SideEffectList) (id' : nat) (res : ValueSequence + Exception)
                          (eff' : SideEffectList),
                        fbs_expr clock env modules id e eff = Result id' res eff' ->
                        exists l : list (SideEffectId * list Value), eff' = eff ++ l)
  ->
fbs_case el env modules id0 eff vals (fbs_expr clock) = Result id' res eff'
->
exists l, eff' = eff ++ l.
Proof.
  induction el; intros.
  ...
  * simpl in H0. destruct a, p. destruct (match_valuelist_to_patternlist vals l) eqn:D1.
    - destruct (fbs_expr clock (add_bindings (match_valuelist_bind_patternlist vals l) env) modules id0 e0 eff) eqn:D2.
...
forall {clock env modules id e eff id' res eff'},
  fbs_expr clock env modules id e eff = Result id' res eff'
->
  exists l, eff' = eff ++ l.
Proof.
  ...
  * destruct (get_value env (inr f)). inversion H1. auto. congruence.
  * destruct (fbs_expr clock env modules id e2 eff) eqn:D1.
    destruct res0. destruct v. congruence. destruct v0. 2, 4-5: congruence.
    - apply IHclock in D1. destruct D1.
      destruct (fbs_expr clock env modules id0 e1 eff0) eqn:D2.
  ...
  * destruct (fbs_values (fbs_expr clock) env modules id l eff) eqn:D1.
    ...
    - inversion H1. subst. eapply effect_extension_exprlist_helper in D1. destruct D1.
      exists x. subst. auto. intros. eapply IHclock. exact H0.
  * destruct (fbs_expr clock env modules id e eff) eqn:D1.
    destruct res0. destruct v. congruence. destruct v0. 2,4-5: congruence.
    - apply IHclock in D1. destruct D1. subst.
      destruct (fbs_values (fbs_expr clock) env modules id0 l (eff ++ x)) eqn:D2.
      destruct res0. 3-4: congruence.
      + destruct v.
  ...
  * destruct (fbs_expr clock env modules id e eff) eqn:D1.
    ...
    - inversion H1. apply IHclock in D1. destruct D1. subst.
      exists x. auto.
  * destruct (fbs_expr clock env modules id e1 eff) eqn:D1.
  ...
  * destruct (fbs_values (fbs_expr clock) env modules id (make_map_exps l) eff) eqn:D1.
    destruct res0. 3-4: congruence.
...
forall env modules id0 eff v id' res eff' clock,
fbs_case l env modules id0 eff v (fbs_expr clock) = Result id' res eff'
->
exists l', eff' = eff ++ l'.
Proof.
  intros.
...
Lemma effect_irrelevant_exprlist_helper el :
  forall env modules id eff id' res eff' clock,
  (forall (env : Environment) (id : nat) (e : Expression) 
            (eff : SideEffectList) (id' : nat) (res : ValueSequence + Exception)
            (eff' : list (SideEffectId * list Value)),
          fbs_expr clock env modules id e eff = Result id' res (eff ++ eff') ->
          forall eff0 : SideEffectList,
          fbs_expr clock env modules id e eff0 = Result id' res (eff0 ++ eff'))
  ->
  fbs_values (fbs_expr clock) env modules id el eff = Result id' res (eff ++ eff')
->
  forall eff0, fbs_values (fbs_expr clock) env modules id el eff0 = Result id' res (eff0 ++ eff').
Proof.
  induction el; intros.
  * simpl in *. inversion H0. rewrite <- app_nil_r in H4 at 1. apply app_inv_head in H4.
    subst. rewrite app_nil_r. reflexivity.
  * simpl in H0. simpl. destruct (fbs_expr clock env modules id a eff) eqn:D1.
    destruct res0. destruct v. congruence. destruct v0. 2: congruence.
    3-4: congruence.
    - remember D1 as D1'. clear HeqD1'. pose (effect_extension_expr D1).
      destruct e. subst.
      apply H with (eff0 := eff0) in D1'. rewrite D1'.
      destruct (fbs_values (fbs_expr clock) env modules id0 el (eff ++ x)) eqn:D2.
      destruct res0. 3-4: congruence.
      + inversion H0. subst.
        ...
        epose (IHel env modules id0 (eff ++ x) id' _ _ clock H D2 (eff0 ++ x)).
...
forall env modules id0 eff v id' res eff' clock,
fbs_case l env modules id0 eff v (fbs_expr clock) = Result id' res (eff ++ eff')
->
(forall (env : Environment) (id : nat) (e : Expression) 
            (eff : SideEffectList) (id' : nat) (res : ValueSequence + Exception)
            (eff' : list (SideEffectId * list Value)),
          ...
          fbs_expr clock env modules id e eff0 = Result id' res (eff0 ++ eff'))
->
forall eff0, fbs_case l env modules id0 eff0 v (fbs_expr clock) = Result id' res (eff0 ++ eff').
Proof.
  induction l; intros.
  ...
  * simpl in *. destruct a, p.
    ...
    - destruct (fbs_expr clock (add_bindings (match_valuelist_bind_patternlist v l0) env) modules id0 e0 eff) eqn:D1.
  ...
  * destruct e; subst.
    ...
    - simpl in H. simpl.
      ...
      + remember D1 as D1'. clear HeqD1'.
        ...
        destruct (fbs_expr clock env modules id0 e1 (eff ++ x)) eqn:D2.
        destruct res0. destruct v0. congruence. destruct v1. 2: congruence.
        3-4: congruence.
    ...
    - simpl. simpl in H. destruct (fbs_values (fbs_expr clock) env modules id l eff) eqn:D1.
      destruct res0. 3-4: congruence.
    ...
    -  simpl. simpl in H. destruct (fbs_values (fbs_expr clock) env modules id l eff) eqn:D1.
      ...
      + remember D1 as D1'. clear HeqD1'.
        ...
        **  remember H as H'. clear HeqH'.
            ...
            replace (eff ++ x ++ x0) with ((eff ++ x) ++ x0) in H'.
            apply IHclock with (eff0 := eff0 ++ x) in H'.
        ...
        **  remember H as H'. clear HeqH'.
            ...
            rewrite <- app_assoc in P. apply app_inv_head in P. subst.
            inversion H. 
            rewrite app_assoc in H1.
            epose proof (P := @eval_effect_irrelevant_snd _ _ _ (eff ++ x) l' H1 (eff0 ++ x)).
            rewrite P. rewrite <- app_assoc. erewrite eval_effect_irrelevant_fst. reflexivity.
        ** intros. apply IHclock with (eff0 := eff2) in H0. auto.
    ...
    - simpl. simpl in H. destruct (fbs_values (fbs_expr clock) env modules id l eff) eqn:D1.
      ...
      + inversion H. subst.
        eapply effect_irrelevant_exprlist_helper in D1. rewrite D1. reflexivity.
        intros. eapply IHclock. exact H0.
    - simpl. simpl in H. destruct (fbs_expr clock env modules id e eff) eqn:D1.
      ...
      + inversion H. subst. eapply IHclock in D1. rewrite D1.
        reflexivity.
    - simpl in H. simpl. destruct (fbs_expr clock env modules id e eff) eqn:D1.
      destruct res0. 3-4: congruence.
      + remember D1 as D1'. clear HeqD1'.
        eapply effect_extension_expr in D1. destruct D1. subst.
        eapply IHclock in D1'. rewrite D1'.
    ...
    - simpl in H. simpl. destruct (fbs_expr clock env modules id e1 eff) eqn:D1.
      destruct res0. 3-4: congruence.
      + remember D1 as D1'. clear HeqD1'.
        eapply effect_extension_expr in D1. destruct D1. subst.
        eapply IHclock in D1'. rewrite D1'.
...
Theorem effect_irrelevant_exprlist {el} :
  forall {env modules  id eff id' res clock eff'},
  fbs_values (fbs_expr clock) env modules id el eff = Result id' res (eff ++ eff')
->
  forall eff0, fbs_values (fbs_expr clock) env modules id el eff0 = Result id' res (eff0 ++ eff').
Proof.
...
Proof.
  unfold weakly_equivalent_expr. split; intros env modules eff res eff' id1 id2 F H1;
  destruct H, H0.
...
Proof.
  unfold weakly_equivalent_exprlist. split; intros env modules eff res eff' id1 id2 F H.
...
Proof.
  unfold weakly_equivalent_exprlist. split; intros env modules eff res eff' id1 id2 F H0.
...
Proof.
  unfold weakly_equivalent_exprlist. split; intros env modules eff res eff' id1 id2 F H1;
  destruct H, H0.
  * apply H in H1. destruct H1, H1. apply H0 in H1. destruct H1, H1. eexists.
    split. exact H1. eapply Permutation_trans. exact H4. exact H5. all : auto.
  * apply H3 in H1. destruct H1, H1. apply H2 in H1. destruct H1, H1. eexists.
...
Proof.
  unfold weakly_equivalent_case. split; intros env moudles eff res eff' id1 id2 vals F H.
  * exists eff'. split. exact H. apply Permutation_refl.
  * exists eff'. split. exact H. apply Permutation_refl.
Qed.
...
Proof.
  unfold weakly_equivalent_case. split; intros env moudles eff res eff' id1 id2 vals F H0.
  * destruct H. apply H1 in H0. exact H0. auto.
  * destruct H. apply H in H0. exact H0. auto.
Qed.
...
Proof.
  unfold weakly_equivalent_case. split; intros env moudles eff res eff' id1 id2 vals F H1;
  destruct H, H0.
  * apply H in H1. destruct H1, H1. apply H0 in H1. destruct H1, H1. eexists.
    split. exact H1. eapply Permutation_trans. exact H4. exact H5. all : auto.
  * apply H3 in H1. destruct H1, H1. apply H2 in H1. destruct H1, H1. eexists.
...
Proof.
  assert (A : forall hd hd' tl tl' env modules id eff id' res eff', P modules ->
    weakly_equivalent_expr P hd hd' -> weakly_equivalent_expr P tl tl' ->
    (exists clock, fbs_expr clock env modules id (ECons hd tl) eff = Result id' res eff')
  ->
    (exists eff'', (exists clock, fbs_expr clock env modules id (ECons hd' tl') eff = Result id' res eff'') /\ Permutation eff' eff'')
  ). {
    intros hd0 hd' tl0 tl' env modules id eff id' res eff' F H H0 H1.
    unfold weakly_equivalent_expr in H, H0. destruct H, H0. destruct H1; subst.
    destruct x; inversion H1. destruct (fbs_expr x env modules id tl0 eff) eqn:D1.
    destruct res0. destruct v. congruence. destruct v0. 2,4-5: congruence.
    * apply effect_extension_expr in D1 as D1'. destruct D1'. subst.
      ...
      destruct (fbs_expr x env modules id0 hd0 (eff ++ x0)) eqn:D2.
      destruct res0. destruct v0. congruence. destruct v1. 2,4-5: congruence.
      - inversion H5. subst.
  ...
  split; intros env modules eff res eff' id1 id2 F H1.
...
Proof.
  assert (A : forall (l l' : list Expression) env modules id eff id' res eff', P modules ->
    ...
    ->
      exists eff'', (exists clock, fbs_expr clock env modules id (ETuple l') eff = Result id' res eff'') /\ Permutation eff' eff''). 
  {
  ...
  split; intros env modules eff res eff' id1 id2 F H0.
  * eapply A. auto. exact H. exact H0.
  * eapply A. auto. exact (weakly_equivalent_exprlist_sym P H). exact H0.
Qed.

Theorem ECall_weak_congr (P : list ErlModule -> Prop) (m : string) (f : string) (l : list Expression) : forall (l' : list Expression),
  weakly_equivalent_exprlist P l l'
->
  weakly_equivalent_expr P (ECall m f l) (ECall m f l').
Proof.
  assert (A : forall (l l' : list Expression) f env modules id eff id' res eff', P modules ->
      weakly_equivalent_exprlist P l l' ->
      (exists clock, fbs_expr clock env modules id (ECall m f l) eff = Result id' res eff')
    ->
      exists eff'', (exists clock, fbs_expr clock env modules id (ECall m f l') eff = Result id' res eff'') /\ Permutation eff' eff''). 
  {
    intros l0 l' f0 env modules id eff id' res eff' F H H0.
    destruct H0, x.
    inversion H0.
    simpl in H0. destruct (fbs_values (fbs_expr x) env modules id l0 eff) eqn:D1.
    destruct res0. 3-4: congruence.
    * destruct get_modfunc eqn: MOD. destruct t, p, f1.
     - 
        ...
        apply effect_extension_exprlist in H1 as [eff'' D]. subst.
        apply effect_extension_expr in H0 as [eff1 P1]. subst.
        eapply effect_irrelevant_expr with (eff2 := eff ++ eff'')  in H0'.
        eapply bigger_clock_list with (clock' := x + x1) in H1'.
        exists ((eff ++ eff'') ++ eff1). split.

        exists (S (x + x1)). simpl. rewrite H1'. rewrite MOD.
        eapply bigger_clock_expr with (clock' := x + x1)  in H0'.
        auto. lia.  perm_solver. lia. intros; apply clock_increase_expr; auto.
        auto. 
     ...
     - inversion H0. subst.
      ...
      exists (x0 ++ x2). split. exists (S x1). simpl. rewrite H1. rewrite MOD. 
      + rewrite Heqf1.
...
Theorem EPrimOp_weak_congr (P : list ErlModule -> Prop) (m : string) (f : string) (l : list Expression) : forall (l' : list Expression),
  weakly_equivalent_exprlist P l l'
->
  weakly_equivalent_expr P (EPrimOp m f l) (EPrimOp m f l').
Proof.
  assert (A : forall (l l' : list Expression) f env modules id eff id' res eff', P modules ->
      weakly_equivalent_exprlist P l l' ->
      (exists clock, fbs_expr clock env modules id (EPrimOp m f l) eff = Result id' res eff')
    ->
      exists eff'', (exists clock, fbs_expr clock env modules id (EPrimOp m f l') eff = Result id' res eff'') /\ Permutation eff' eff''). 
  {
    intros l0 l' f0 env modules id eff id' res eff' F H H0.
    destruct H0, x.
    inversion H0.
    ...
    * inversion H0. subst.
      ...
      erewrite eval_effect_irrelevant_fst. reflexivity. subst. perm_solver. auto.
    * inversion H0. subst.
      apply ex_intro with (x := x), H in D1. destruct D1, H1, H1.
      exists x0. split. exists (S x1). simpl. rewrite H1. all :auto.
...
Proof.
  assert (A : forall exp exp' (l l' : list Expression) env modules id eff id' res eff', P modules ->
    weakly_equivalent_expr P exp exp' ->
    weakly_equivalent_exprlist P l l'
    ->
    (exists clock, fbs_expr clock env modules id (EApp exp l) eff = Result id' res eff')
    ->
    exists eff'', (exists clock, fbs_expr clock env modules id (EApp exp' l') eff = Result id' res eff'') /\ Permutation eff' eff'').
  {
...
Proof.
  assert (A : forall exp exp' (l l' : list (list Pattern * Expression * Expression)) env modules id eff id' res eff', P modules ->
  weakly_equivalent_expr P exp exp' ->
    weakly_equivalent_case P l l'
  ->
  ...
  ->
    (exists eff'', (exists clock,
                   fbs_expr clock env modules id (ECase exp' l') eff = Result id' res eff'') /\
                   Permutation eff' eff'') 
  ).
  {
    intros exp0 exp' l0 l' env modules id eff id' res eff' F H H0 H1. 
...
Proof.
  assert (A : forall env modules id e1 e1' e2 e2' vl eff id' res eff', P modules ->
  ...
  ->
    (exists eff'', (exists clock, fbs_expr clock env modules id (ELet vl e1' e2') eff = Result id' res eff'') /\ Permutation eff' eff'')
  ).
  {
    intros env modules id e0 e1' e3 e2' vl0 eff id' res eff' F H H0 H1.
...
Proof.
  assert (A : forall env modules id e1 e1' e2 e2' eff id' res eff', P modules ->
    weakly_equivalent_expr P e1 e1' -> weakly_equivalent_expr P e2 e2' ->
    (exists clock, fbs_expr clock env modules id (ESeq e1 e2) eff = Result id' res eff')
  ->
    (exists eff'', (exists clock, fbs_expr clock env modules id (ESeq e1' e2') eff = Result id' res eff'') /\ Permutation eff' eff'')
  ).
  {
    intros env modules id e0 e1'0 e3 e2'0 eff id' res eff' F H H0 H1.
    destruct H1, x. inversion H1. simpl in H1.
    destruct (fbs_expr x env modules id e0 eff) eqn:D1.
    destruct res0. destruct v. congruence. destruct v0. 2,4-5: congruence.
    * apply effect_extension_expr in D1 as D1'. destruct D1'. subst.
...
Proof.
  assert (A :
    forall (l l' : list (Expression * Expression)) env modules id eff eff' id' res, P modules ->
    weakly_equivalent_exprlist P (make_map_exps l) (make_map_exps l') ->
    (exists clock, fbs_expr clock env modules id (EMap l) eff = Result id' res eff')
    ->
    (exists eff'', (exists clock, fbs_expr clock env modules id (EMap l') eff = Result id' res eff'') /\ Permutation eff' eff'')
  ). {
    intros l0 l' env modules id eff eff' id' res F H H0.
    destruct H0, x. inversion H0. simpl in H0.
    destruct (fbs_values (fbs_expr x) env modules id (make_map_exps l0) eff) eqn: D1.
    destruct res0. 3-4: congruence.
...
Proof.
  assert (A : forall env modules id e1 e1' e2 e2' e3 e3' eff id' res eff', P modules ->
    weakly_equivalent_expr P e1 e1' ->
    weakly_equivalent_expr P e2 e2' ->
    weakly_equivalent_expr P e3 e3' ->
    (exists clock, fbs_expr clock env modules id (ETry e1 vl1 e2 vl2 e3) eff = Result id' res eff')
  ->
    (exists eff'', (exists clock, fbs_expr clock env modules id (ETry e1' vl1 e2' vl2 e3') eff = Result id' res eff'') /\ Permutation eff' eff'')
  ).
  {
    intros env modules id e0 e1' e4 e2' e5 e3' eff id' res eff' F H H0 H1 H2.
  ...
  split; intros env modules eff res eff' id1 id2 F H2.
...
Proof.
  assert (A : forall (l l' : list Expression) env modules id eff id' res eff', P modules ->
    ...
    ->
      exists eff'', (exists clock, fbs_expr clock env modules id (EValues l') eff = Result id' res eff'') /\ Permutation eff' eff'').
  {
    intros l0 l' env modules id eff id' res eff' F H H0.
    destruct H0, x.
    inversion H0.
