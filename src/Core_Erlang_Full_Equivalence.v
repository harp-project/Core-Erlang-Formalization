...
Theorem EValues_full_cong (l : list SingleExpression) : forall (l' : list SingleExpression),
  fully_equivalent_singlelist l l'
->
...
Proof.
    ...
    (exists clock, fbs_expr clock env id (EValues l') eff = Result id' res eff')).
  {
   intros. unfold fully_equivalent_exprlist, fully_equivalent_single in *.
    destruct H0. destruct x. inversion H0. simpl in H0.
    destruct (fbs_values (fbs_single x) env id l0 eff) eqn:D.
    destruct res0. 3-4: congruence.
    * inversion H0. subst. epose (E := H _ _ _ _ _ _). destruct E.
      pose (E := H1 (ex_intro _ x D)). destruct E. exists (S x0). simpl. rewrite H3. auto.
    * inversion H0. subst. epose (E := H _ _ _ _ _ _). destruct E.
  ...
  split; intros.
  * eapply A. exact H. auto.
  * eapply A. exact (fully_equivalent_singlelist_sym H). exact H0.
Qed.

Theorem ESingle_full_cong (e : SingleExpression)  : forall (e': SingleExpression),
  fully_equivalent_single e e'
->
  fully_equivalent_expr (ESingle e) (ESingle e').
Proof.
  intros.
  split; intros.
  * destruct H0, x. inversion H0. simpl in H0.
...
(** 
  ...
  * *)
Goal
  fully_equivalent_single (ETuple [^EApp ErrorExp []]) (ETuple [^ErrorExp;^EApp ErrorExp []]).
Proof.
  ...
  * clear H1. exists (S x).
    ...
    simpl in H; destruct x; inversion H.
    simpl in H; destruct x; inversion H.
    simpl in H; destruct x; inversion H. simpl in H. inversion H. clear H1. clear H2. clear H3.
    simpl. auto.
  * clear H1. exists (S x).
    simpl in H; destruct x; inversion H.
    simpl in H; destruct x; inversion H.
...
Goal
  fully_equivalent_single (ETuple [^ErrorExp; ^EApp ErrorExp []]) (ETuple [^ErrorExp;^EApp ErrorExp []]).
Proof.
