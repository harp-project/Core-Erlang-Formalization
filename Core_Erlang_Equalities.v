...
(** Boolean equalities: *)
  ...
  match e1, e2 with
   ...
   | ECase e pl gl bl, ECase e' pl' gl' bl' => bExpression_eq_dec e e' && 
   ...
   | ELet s el e, ELet s' el' e' => (fix blist l l' := match l, l' with
                                                 | [], [] => true
                                                 | x::xs, x'::xs' => andb (eqb x x') (blist xs xs')
                                                 | _, _ => false
                                                 end) s s' &&
   ...
   | ELetrec fnames varlists bodylists e, ELetrec fnames' varlists' bodylists' e' => 
                                               (fix blist l l' := match l, l' with
                                                 | [], [] => true
                                               ...
                                               bExpression_eq_dec e e'
   | EMap kl vl, EMap kl' vl' => (fix blist l l' := match l, l' with
                                                 | [], [] => true
                                                 | x::xs, x'::xs' => andb (bExpression_eq_dec x x') (blist xs xs')
                                                 | _, _ => false
                                                 end) kl kl' &&
   ...
   | ETry e e1 e2 v1 vex1 vex2 vex3, ETry e' e1' e2' v1' vex1' vex2' vex3' => bExpression_eq_dec e e' && bExpression_eq_dec e1 e1' && bExpression_eq_dec e2 e2' && eqb v1 v1' && eqb vex1 vex1' && eqb vex2 vex2' && eqb vex3 vex3'
   | _, _ => false
  end.
  ...
  | VMap kl vl, VMap kl' vl' => (fix blist l l' := match l, l' with
                                                   | [], [] => true
                                                   | x::xs, x'::xs' => andb (bValue_eq_dec x x') (blist xs xs')
                                                   | _, _ => false
                                                   end) kl kl' &&
...
Import Structures.OrderedTypeEx.String_as_OT.
  ...
  | lt_tuple_tuple_hd exps exps' hd hd' : lt_Value hd hd' -> lt_Value (VTuple (hd::exps)) (VTuple (hd'::exps'))
  | lt_tuple_tuple_tl exps exps' hd hd' : hd = hd' -> lt_Value (VTuple exps) (VTuple exps') -> lt_Value (VTuple (hd::exps)) (VTuple (hd'::exps'))
  ...
  | lt_map_map_hd (kl kl' vl vl' : list Value) hd hd': length (hd'::kl') = length vl' -> length (hd::kl) = length vl -> lt_Value hd hd' -> lt_Value (VMap (hd::kl) vl) (VMap (hd'::kl') vl')
  | lt_map_map_vl (kl kl' vl vl' : list Value) hd hd' hdv hdv': length (hd'::kl') = length (hdv'::vl') -> length (hd::kl) = length (hdv::vl) -> hd = hd' -> lt_Value (VMap kl vl) (VMap kl' vl') -> lt_Value (VMap (hd::kl) (hdv::vl)) (VMap (hd::kl') (hdv'::vl'))
  ...
  | VLiteral _, _ => true
  (* TODO: This needs to be extended?? *)
  | VClosure _ _ _ _, VClosure _ _ _ _ => false
  ...
  | VTuple l, VTuple l' => list_less Value value_less bValue_eq_dec l l'
  | VTuple _, VEmptyList => true
  | VTuple _, VMap _ _ => true
  | VTuple l, VList _ _ => true
  | VMap kl vl, VMap kl' vl' => list_less Value value_less bValue_eq_dec kl kl'
